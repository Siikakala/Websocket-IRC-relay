...
if($irc->connect()){
    ...
    while(!feof($irc->socket)){ //pysytään silmukassa, kunnes yhteys katkee jostain syystä.
        ...
        if($irc->initialize($data) == true){ //jollei ole PING, jatkokäsittely
			...
			if(strstr($msg,"KICK") && strstr($msg,$botnick)){sleep(5);$irc->join($kanava);} //rejoin-on-kick, mutta odotetaan 5sec välissä.
			...
			if($priva == 0)$command = substr($command[0],1);else $command = substr($command[0],0); //ja tiputetaan loput viestistä (priva huomioitu)
			if(!$ignore){
				$c = array_search($command,$commands,true); //katsotaan löytyykö komentoo hyväksytyistä komennoista.
				if($c !== false){print "Command ".$commands[$c]." spawned\n";}else{print"False command, ignoring.. \n"; $c = null;} //Kerrotaan mikä komento löytyi, ja jollei komentoa löydy, tulostetaan False command
