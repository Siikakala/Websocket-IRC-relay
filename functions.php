	...
	if($c !== false){
		switch($allowed[$c]){
				...
				if(mobilenro($nick,false) && isset($cmds[2]) == false){ //tarkistetaan toisella funktiolla, onko numero jo kannassa, ja halutaanko sitä muuttaa
					...
					if(is_numeric($cmds[1]) && strlen($cmds[1]) == 10){//jos toinen attribuutti ei ole numero ja mikäli siinä on välejä (se ei ole kymmenen merkkiä pitkä), huudetaan
						if(strstr($cmds[2],"muutos")){
							...
							if(is_numeric($cmds[2]))$return="Syötä numero ilman välilyöntejä";else$return = "Viimeinen sana ei ole \"muutos\".";
						...
						if($sql != null){
							...
							if (mysql_errno() != 0) {
								print mysql_error()."\n";
								if($muutos)$return = "Numeron muokkaus failasi.";else$return = "Numeron lisäys failasi.";
							}else{
								if($muutos)$return = "$nick:n numeron muokkaus onnistui! (".$cmds[1].")";else$return = "$nick:n numero (".$cmds[1].") lisättiin kantaan.";
							}
						}
					}else{
			...
			case "deadline":
				...
				if(empty($put["time"]) || empty($put["day"]) || empty($put["msg"])) { $return = "Tietoja puuttuu (päivä, aika tai viesti)"; $addlock = true;} //jos tietoja puuttuu, addlock päälle
			...
			case "pysäkki":
				...
				if($olemassa && isset($cmds[3]) == false){ //tarkistetaan toisella funktiolla, onko numero jo kannassa, ja halutaanko sitä muuttaa
					...
					if($sql != null){
						...
						if (mysql_errno() != 0) {
							print mysql_error()."\n";
							if($muutos)$return = "Pysäkin muokkaus failasi.";else$return = "Pysäkin lisäys failasi.";
						}else{
							if($muutos)$return = "$nick:n pysäkin muokkaus onnistui! (".$cmds[1]." == ".$cmds[2].")";else$return = "$nick:n pysäkki (".$cmds[1]." == ".$cmds[2].") lisättiin kantaan.";
						}
					}else
